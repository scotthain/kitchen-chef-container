# Contributing to Kitchen-Chef-Container

We are glad you want to contribute to Kitchen-Chef-Container!
...
3. Do [Code Review](#cr) with the **Chef Engineering Team** or **Kitchen-Chef-Container Core Committers** on the pull request.
...
1. **Tests:** To ensure high quality code and protect against future regressions, we require all the
  code in Chef to have at least unit test coverage. See the [spec/unit](https://github.com/opscode/chef-init/tree/master/spec/unit)
...
Once you a pull request, the **Chef Engineering Team** or **Kitchen-Chef-Container Core Committers** will review your code
  and respond to you with any feedback they might have. The process at this point is as follows:

1. 2 thumbs-ups are required from the **Chef Engineering Team** or **Kitchen-Chef-Container Core Committers** for all merges.
...
Otherwise you can file your issue in the [Kitchen-Chef-Container project](https://github.com/opscode/kitchen-chef-container/issues)
...
## <a name="release"></a> Kitchen-Chef-Container Release Cycles

We also release our software as gems to [Rubygems](http://rubygems.org/) but we strongly
  recommend using Kitchen-Chef-Container packages since they are the only combination of native libraries &
  gems required by Kitchen-Chef-Container that we test throughly.
